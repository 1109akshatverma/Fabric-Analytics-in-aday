...
5. **첫 번째 보고서 화면 만들기** 로 이동됩니다. Excel, CSV를 사용하여
    ...
    모델을 선택할 수 있는 옵션이 제공됩니다. 이전 랩에서 의미 체계
...
10. 보고서 저장 대화 상자가 열립니다. 보고서 이름을 **rpt_Sales_Auto_Report** 로 지정합니다. **참고:** 보고서 이름 앞에는 보고서의 약자인 rpt가 붙습니다.
...
5. **보고서 자동 만들기 버튼** 옆에 있는 화살표를 클릭합니다. **빈 보고서 만들기를** 을 선택합니다. 보고서 페이지와 비슷한 모양의 Power BI Desktop 보고서 페이지로 이동합니다.
...
4. 오른쪽의 **데이터 패널에서** 테이블을 선택합니다.

5. **Date** 테이블을 확장합니다.

6. **Year** 열을 선택합니다.

7. 오른쪽 **속성** 창에서 **고급** 섹션을 확장합니다.
...
10. 오른쪽의 **데이터 창** 에서 **Date** 테이블을 펼칩니다. Year는 합계
    필드가 아닙니다.

11. **꺾은선형 차트 시각적 개체를 선택한** 상태에서 Y축에서 **Year 합계** 를 제거합니다.

12. **Year** 필드를 선택하면 **X축** 에 추가됩니다.
...
17. 보고서를 저장하겠습니다. 메뉴에서 **파일 -> 저장** 을 선택합니다.
    ...
    시간이 있다면 자유롭게 더 많은 시각 요소를 빌드해보십시오.
...
17. 이를 수정해 보겠습니다. **시각화** 패널의 **X축** 아래에서 **StartOfMonth** 를 삭제합니다.
...
ADLS Gen2에서 데이터가 업데이트되고 새로 고침을 실행하지 않고도 변경
...
2. 오른쪽 하단 모서리에서 **Fabric** **로고** 를 클릭하고 **Fabric
    뷰**로 전환합니다.

3. 왼쪽 메뉴 모음에서 **FAIAD_<inject key="Deployment ID" enableCopy="false"/>** 을 선택하여 작업 영역
    홈으로 이동합니다.
...
5. **왼쪽 탐색기 창**에서 **테이블** 옆의 **줄임표** 를 선택합니다.

6. **새 바로 가기** 를 선택합니다.

    ![](../media/lab-7/image45.png)

7. 새 바로 가기 대화 상자가 열립니다. **외부 소스** 에서 **Azure Data Lake Storage Gen2** 를 선택합니다.

    ![](../media/lab-7/image46.png)

8. 랩에서 앞서 연결을 생성했으므로 새 연결을 생성하지 않아도 되며, 기존
    연결에서 ADLS 연결을 확인할 수 있습니다.

9. 이 과정에서 앞서 연결을 생성하지 않았다면 **새 연결 생성** 을
    클릭하고 다음 과정을 완료하십시오.

10. **연결 설정 -> URL** 에서 다음 링크를 입력합니다.
...
13. **Sales.Invoices_May** 를 선택합니다.

14. **다음** 을 선택합니다.

    ![](../media/lab-7/image48.png)

15. 이름을 편집할 수 있는 다음 대화 상자로 이동합니다.
    **Sales.Invoices_May** 에 대한 작업에서 **편집 아이콘** 을 선택합니다.

16. **Sales.Invoices_May의 이름을 InvoicesMay** 로 변경합니다.

17. 이름 옆의 **확인 표시** 를 선택하여 변경 사항을 저장합니다.
...
19. 화면 **오른쪽 상단** 에서 **레이크하우스 -> SQL 분석 엔드포인트** 를
    선택합니다.

    ![](../media/lab-7/image50.png)

20. 상단 메뉴에서 **홈 - > 새 SQL 쿼리** 를 선택합니다. 새 SQL 쿼리 창이
    열립니다.

21. 아래 코드를 **복사** 하여 SQL 쿼리 창에 **붙여넣습니다**.
...
22. 시각적 개체 쿼리 메뉴에서 **실행** 을 선택하여 코드를 실행합니다.
...
23. 왼쪽 메뉴 모음에서 **rpt_Sales_Report** 를 선택하여 보고서로 다시
    이동합니다 **.**

24. 상단 메뉴에서 **새로 고침** 을 선택합니다. 이제 꺾은선형 차트에
...
1. 왼쪽 패널에서 **FAIAD_<inject key="Deployment ID" enableCopy="false"/>** 작업 영역을 선택하여 작업 영역
    홈으로 이동합니다.
