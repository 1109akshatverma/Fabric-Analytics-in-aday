...
- **직원 데이터:** SharePoint에서 매일 오전 9시에 업데이트됩니다. 그러나
  때로는 5~15분 정도 지연되는 경우도 있습니다. 이에 대응하려면 새로
  고침 일정을 만들어야 합니다.

- **Customer 데이터:** Dataverse에서 항상 최신 상태로 유지됩니다.
  이전에는 자정/오전 12시, 오전 6시, 정오/오후 12시, 오후 6시 등 하루에
  네 번 새로 고쳤습니다. 이제 IT 팀은 이 데이터를 Admin 레이크하우스로
  수집하기 위해 Dataverse에 대한 링크를 만들었습니다. 또한 이 데이터를
  변환했습니다. IT팀에서 제공하는 레이크하우스에 연결하므로 새로 고침을
  설정할 필요가 없습니다.
...
3. 생성한 모든 아티팩트가 여기에 나열됩니다. 화면 오른쪽에 있는 **검색 창** 에 **df** 를 입력합니다. 그러면 아티팩트가 데이터 흐름으로 필터링됩니다.

    ![](../media/lab-5/image7.png)

4. **df_Supplier_Snowflake** 행 위에 마우스를 올립니다. **줄임표** 를
    선택합니다.

5. 데이터 흐름을 삭제, 열기, 새로 고침할 수 있는 옵션이 있습니다. 새로
    고침 기록을 살펴보겠습니다. **최근 실행을** 선택합니다.

    ![](../media/lab-5/image8.png)

    **참고:** 오른쪽에 새로 고침 목록을 보여주는 창/패널이 나타납니다.

6. 이전 랩에서 **저장 후 실행** 옵션을 선택했을 때 단일 새로 고침이
    실행된 것을 확인할 수 있습니다. 확인 가능한 새로 고침 **형식은 요청
    시로** 표시되어 있으며, 이를 통해 수동으로 실행된 새로 고침임을 알
    수 있습니다.
...
8. 오른쪽 상단에서 **X** 를 클릭하면 화면이 닫힙니다. **작업 영역** 으로
    다시 이동합니다.

9. **df_Supplier_Snowflake** 행 위에 마우스를 올립니다. **줄임표** 를
    선택합니다.

10. 자동으로 새로 고침이 실행되도록 일정을 설정하는 방법을
    알아보겠습니다. **설정** 옵션을 선택합니다.

    ![](../media/lab-5/image12.png)

11. **설정** 패널에 나타난 다음 세 가지 옵션을 확인할 수 있습니다
    **정보 --** 여기서 데이터 흐름의 이름을 변경하고 설명을 추가할 수
    있습니다. 또한 또한 데이터 흐름의 담당자와 마지막 수정 시점을 확인할
    수 있습니다
    **보증 --** 이를 통해 데이터 흐름이 다른 사용자에게 표시될 때
    **승격됨** 태그를 적용할지 **인증됨** 태그를 적용할지 지정할 수
    있습니다.
    **일정 --** 데이터 흐름을 예약할 수 있는 위치입니다.

    ![](../media/lab-5/image13.png)

12. **일정** 옵션을 선택합니다.

13. 일정을 활성화하려면 단순히 **일정 추가** 를 클릭하면 됩니다.

    ![](../media/lab-5/image14.png)

14. 이제 **반복** 속성에 대한 옵션을 선택하여 새로 고침의 주기를 지정할
    수 있습니다. 이 시나리오에서는 **일일 (1)** 을 선택할 수 있습니다.

15. **시간** 속성의 경우 자정을 의미하므로 **오전 12시 (2)** 를 지정할 수
    있습니다.

    > **참고:** 다른 시간 추가 링크를 클릭하면 새로 고침 시간을 여러 개 추가할 수 있습니다.

16. **시작 날짜 및 시간(3)** 과 **종료 날짜 및 시간(4)** 도 지정할 수
    있습니다. 이 시나리오에서는 시작 날짜와 종료 날짜로 현재 날짜를
    선택하기만 하면 됩니다.

17. 시간을 표시할 **표준 시간대 (5)** 를 지정할 수 있습니다. 마지막으로
    **저장** 을 선택합니다.

    ![](../media/lab-5/image15.png)

18. 예약된 새로 고침을 확인하고, 더 이상 필요하지 않은 경우 편집하거나
    삭제할 수 있으며, 다른 예약된 새로 고침을 추가할 수도 있습니다.
...
3. 새 파이프라인 대화 상자가 열립니다. 파이프라인의 이름을
    **pl_Refresh_People_SharePoint** 로 지정하고 **만들기** 를 선택합니다.

    ![](../media/lab-5/image18.png)

    **파이프라인 페이지로 이동되었습니다.** Azure Data Factory를 사용해 본
    적이 있다면 이 화면이 익숙할 것입니다. 레이아웃에 대한 간략한 개요를
    살펴보겠습니다.
...
7. 상단 메뉴에서 **보기** 를 선택합니다. 여기서는 JSON 형식으로 코드를
    볼 수 있는 옵션을 찾을 수 있습니다. 또한 활동을 자동으로 정렬하는
    옵션도 있습니다.
...
2. df_People_SharePoint 데이터 흐름에 연결하도록 활동을 구성하겠습니다.
    하단 창에서 설정을 선택합니다.
...
13. 메뉴에서 **홈 -> 저장** 아이콘을 선택하여 파이프라인을 저장합니다.
    ...
    - 파이프라인은 새로 고침이 실패하기 전에 여러 번 재시도할 수 있는 옵션을
    제공합니다.

    - 파이프라인은 데이터 흐름을 새로 고치는 것 외에도 다른 작업을 수행할 수
    있는 기능을 제공합니다.
...
2. 상단 메뉴에서 **+ 새 항목 (1)** 을 클릭하고 팝업 창에서 **데이터
    파이프라인 (2)** 을 클릭합니다.
...
6. **기본값** 으로 **0** 을 입력합니다.
    ...
    **참고:** 변수 이름에 var를 붙여 추가하고 있습니다. 이렇게 하면 변수를
    쉽게 찾을 수 있으며, 이는 좋은 관행입니다.
...
11. 유사한 단계에 따라 변수를 세 개 더 추가합니다.
    ...
    c. **varWaitTime** of type **Integer** and default value **60**. 이
...
2. 첫 번째 행에서 **수정 아이콘** 을 선택합니다. 빈 반복기 설계 화면으로
    되돌아갑니다.
...
11. 하단 메뉴에서 **줄임표(...) (2)** 를 클릭합니다. **변수 (3) ->
    varSuccess(4)** 를 선택합니다. **@variables('varSuccess')** 가
...
10. 설계 캔버스의 왼쪽 상단에서 **pl_Refresh_People_Sharepoint_Option2**
    또는 **기본 캔버스** 를 선택하여 Until 반복기에서 빠져나옵니다.

    ![](../media/lab-5/image51.png)

11. 데이터 파이프라인 생성이 완료되었습니다. 상단 메뉴에서 **홈 -> 저장
