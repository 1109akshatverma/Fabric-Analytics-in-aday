...
3. **데이터 가져오기 (3)** 섹션에서 **데이터 흐름 2세대(4)를 선택합니다**.
    ...
    기본 이름을 그대로 두고 만들기를 선택합니다. **데이터 흐름 페이지** 로
    이동됩니다. Dataflow Gen2 인터페이스는 Power BI Desktop의 Power Query와
    유사합니다. 쿼리를 Power BI Desktop에서 데이터 흐름 2세대로 복사할 수
    있습니다. 시도해 보겠습니다.
...
2. 왼쪽 상단 모서리에서 **저장 후 실행 (2)** 을 선택합니다. 새로 고침이
    ...
    **참고: FAIAD_FAIAD_<inject key="Deployment ID" enableCopy="false"/> 작업 영역** 으로 되돌아갑니다. 데이터 흐름이
...
3. 권장 항목에서 **데이터 흐름 2세대 (3)** 을 선택합니다.
    ...
    기본 이름을 그대로 두고 **만들기를** 선택하되, "이 이름의 데이터 흐름은
    이미 존재합니다"라는 메시지가 표시되면 이름을 **Dataflow 2** 로
    변경합니다. 그러면 **데이터 흐름** 페이지로 이동하게 됩니다. 이제 데이터
    흐름에 익숙해졌으므로 Power BI Desktop에서 데이터 흐름으로 쿼리를 복사해
    보겠습니다.
...
3. 데이터 대상에 연결 대화 상자가 열립니다. **연결 드롭다운에서** **Lakehouse odl_user_<사용자 이름>(없음)** 을(를) 선택합니다.
...
4. 왼쪽 상단 모서리에서 **저장 후 실행** **(1)** 을 선택합니다. 새로
    ...
    **FAIAD_<inject key="Deployment ID" enableCopy="false"/> 작업 영역**으로 되돌아갑니다. 데이터 흐름이
...
8. 연결 설정 대화 상자에서 **다음** 을 선택합니다.
    ...
    2세대를 사용하여 SharePoint에 연결할 수도 있습니다.
