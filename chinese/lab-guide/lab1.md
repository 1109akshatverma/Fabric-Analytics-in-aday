...
- **员工数据：** 来自 HR 系统；作为导出文件存储在 SharePoint
  ...
  9 点更新。
    ...
    ![](../media/lab-1/image7.jpeg)

您正在 Power BI Premium
中生成一个语义模型，该模型从上述源系统中提取数据，以满足您的报告需求并为最终用户提供自助服务功能。您使用
Power Query 更新模型。
...
1. 打开 **FAIAD.pbix，**它位于您的实验室环境的**桌面**上的 **Reports**
    文件夹中。该文件将在
    Power BI Desktop 中打开。

    ![](../media/lab-1/image8.png)

2. "输入您的电子邮件地址"对话框随即打开。导航到实验室环境中右侧面板上的**环境详细信息**选项卡。

3. 复制**用户名**，并将其粘贴到对话框的**电子邮件**文本框中。
...
7. 在下一个对话框中，通过从**环境详细信息**选项卡中复制**密码凭据**来输入密码凭据。
...
4. 从屏幕底部中间的散点图视觉对象（按销售额划分的销售订单）中，选择右上角的离群值（第四象限）。请注意，利润率为
    52%，高于 50% 的平均值。此外，2023 年最后两个季度的销售额同比增长。
...
11. 请注意，**ADLSData** 文件夹具有多个维度：Geo、Product、Reseller 和
    ...
    - **Geo 维度**是通过合并 Cities、Countries 和 States
      查询的数据而创建的。

    - **Product 维度**是通过合并 Product Groups 和 Product Item Group
      查询中的数据而创建的。

    - **Reseller 维度**是使用 BuyingGroup 查询筛选而来的。

    - **Sales 事实**是通过合并 InvoiceLineItems 与 Invoice
      查询而创建的。
...
14. 请注意，**SnowflakeData** 文件夹包含 Supplier 维度和
    ...
    - **Supplier 维度**是通过合并 Suppliers 查询与 SupplierCategories
      查询而创建的。
...
17. 请注意，**SharepointData** 文件夹具有 People 维度。
    ...
    Gen2 创建一个类似的 Power Query 并使用湖屋进行建模。
