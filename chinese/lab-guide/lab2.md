...
的各种主要功能。这是一个介绍性研讨会，旨在向您介绍 Fabric
中提供的各种产品体验和项目。在本次研讨会结束时，您将学习如何使用湖屋、数据流
Gen2、数据管道和 DirectLake 等。在本实验室结束时，您将了解到：
...
1. 打开**浏览器**并导航到 [Microsoft Power BI
    门户](https://app.powerbi.com/)。系统会将您导航到登录页面。

    ***注意：** 如果您正在使用实验室环境，您可能会自动登录。*

    ***注意：** 如果您未在使用实验室环境，并且已经有 Power BI
...
11. 系统会将您导航到 **Microsoft Fabric** 主**页**。"欢迎使用 Fabric
    视图"对话框可能会打开。
    如果需要，您可以选择**开**始导览选项或取消。
...
4. 在**名称**字段中，输入 **FAIAD_<inject key="Deployment ID" enableCopy="false"/>**（在"环境"选项卡中找到）

    ***注意：** 工作区名称必须唯一。确保"名称"字段下方显示带有"此名称可用"
    的绿色复选标记。*
...
8. 选择**应用**以创建新工作区。
    ...
    新工作区已创建，您将能够导航到此工作区。我们将来自不同数据源的数据引入湖屋中，并使用湖屋中的数据来生成模型并生成报表。第一步是创建一个湖屋，我们接下来将执行此操作。

## 任务 3：创建湖屋

1. 在新创建的工作区 **FAIAD_Username** 中，在左侧导航窗格中找到 **+
    新建项 (1)** 按钮。
    在此处，您可以开始在工作区中创建新项目。

2. 在搜索框中，键入**湖屋 (2)**，然后从搜索结果中，选择 **湖屋 (3)**
...
3. "新建湖屋"对话框随即显示。在"名称"文本框中输入 **lh_FAIAD**。
    ...
    ***注意：湖屋架构** 的预览功能非常有趣，您应该注意这一点。因为它处于 **预览版** 状态，因此我们将忽略它，以便不会对实验室体验产生负面影响。该功能正式发布后，我们会将其引入本实验室中。*

4. 选择 **创建**

    ![](../media/lab-2/image17.png)

    湖屋很快就会创建完毕，然后系统会将您导航到湖屋资源管理器界面。在左上角，在标头中的
    Fabric
    名称旁边，您将看到"湖屋"图标。左侧导航上的工作区图标将反映它现在包含一个项目

    在湖屋资源管理器中，您将看到 Tables 和 Files
    部分。湖屋的文件部分下可能会显示 Azure Data Lake Storage Gen2
...
1. 选择屏幕左侧的"工作负载"图标。将打开一个包含 Fabric
    ...
    Solutions、Real-Time Intelligence、
...
3. 系统会将您定向到 Data Factory
    ...
    **项目类型：**
    ...
    a) **数据流 Gen2：** 数据流就像转换数据的配方。它们提供 300
    ...
    d) **Apache Airflow 作业：** Apache Airflow
        是一个开放源代码平台，用于以编程方式创作、计划和监视工作流。在 Data
        Factory 中，它允许您创建、计划和管理复杂的数据工作流。

    e) **复制作业：** 复制作业是允许您将数据从一个源复制到另一个源的功能。它提供了一种在不同的数据存储之间移动数据的简单高效的方法。

    f) **镜像数据库：** 用于创建数据库的镜像版本以进行备份、测试或只读访问的功能。

    g) **镜像的 Oracle（预览版）：** Fabric 中的镜像将您的 Oracle
        数据库复制到一个统一平台中，从而能够与其他数据源一起进行准实时、低延迟的分析。

    h) **镜像的 Google Big Query（预览版）：** Fabric 中的镜像可让您将
        Google BigQuery 数据持续复制到 OneLake 中，从而消除复杂的
        ETL，并实现跨分析、AI 和数据共享的无缝使用。

    i) **变量库（预览版）：** 包含变量及其默认值的列表。还可以包含保存备选值的其他值集
...
2. 系统会将您定向到 Industry Solutions 主页。下面是其各部分的详细概述，旨在帮助您有效地逐步使用 Industry Solutions。
    ...
    **入门：** 若要开始使用 Industry Solutions，请按照以下步骤操作：
    ...
    f) **部署零售解决方案：** 单击"部署"按钮以开始部署零售解决方案并在您的项目中 实施它们。
...
2.  系统会将您定向到 Real-Time Intelligence
    ...
    **项目类型：**
    ...
    e. **激活器：** 用于监视数据集、查询和事件流中的模式。

    f. **事件架构集（预览版）：** 帮助您组织和标准化实时分析工作流的数据结构（架构），从而更轻松地以一致方式处理和分析流数据。

    g. 数字**孪生体生成器（预览版）：** 数字孪生体生成器为用户提供低代码/无代码体验，以通过本体生成其业务概念（例如资产和流程）并对其进行建模。

    h. **地图（预览版）：** 将地理空间见解引入 Real-Time
        Intelligence，使任何人都能直观了解事件发生的位置，将空间数据与其他
        Fabric 功能集成，并做出更明智的位置感知决策。

    **入门：** 若要开始使用 Real-Time Intelligence，请按照以下步骤操作：

    a. **实时端到端体验：** 单击"入门"按钮以使用示例数据集探索实时数据分析。

    b. **探索 Real-Time Intelligence
        示例：**单击"打开"按钮以使用示例探索实时数据分析。

    c. **探索 Eventhouse 示例：** 单击"选择"按钮以使用示例并了解 Real-Time
        Intelligence。

    d. **Real-Time Intelligence 简介：** 单击"打开"按钮以获取 Real-Time
        Intelligence 的概述，并开始有效地使用该工具。

    e. **使用示例数据了解 KQL：** 单击"打开"按钮以使用示例数据了解 KQL。

    f. **什么是实时中心：** 单击"打开"按钮以了解实时中心的概念以及使用方式。

    g. **镜像的 Oracle（预览版）：** 用于镜像 Oracle。

    h. **激活器入门：** 单击"打开"按钮以开始使用激活器概念并开始有效地使用该工具。
...
2. 系统会将您定向到 **Data Engineering**
    ...
    **项目类型：**
    ...
    e. **用户数据功能（预览版）：** 允许您在 Fabric
        中托管和运行应用程序的平台。

    f. **API for GraphQL：** 这是用于查询多个数据源的 API。
    ...
    **入门：**
    ...
    g. **将笔记本用于您的湖屋：** 单击"打开"按钮以了解如何将笔记本用于您的湖屋。

    h. **将数据集用于您的湖屋：** 单击"打开"按钮以了解如何将数据集用于您的湖屋。
...
2. 系统会将您定向到 **Data Science** 主页。下面是其各部分的详细概述，旨在帮助您有效地使用 **Data Science**。
    ...
    **什么是 Data Science？**
    ...
    和机器学习技术发现深刻的见解的工具。它提供 AI 工具，
    旨在帮助您完成全面的数据科学工作流，并利用 AI
    进行数据扩充和获取业务见解。
    ...
    **项目类型：**
    ...
    e. **数据代理（预览版）：** 用于创建对话式 AI 体验，以回答有关存储在湖屋、仓库、Power BI 语义模型和 KQL 数据库中的数据的问题

    f. **Python 笔记本：** 用于从本地计算机导入 Python 笔记本。

    **入门：** 若要开始使用 Data Science，请按照以下步骤操作
...
2. 系统会将您定向到 Data Warehouse
    ...
    **项目类型：**
    ...
    e. **镜像的 Azure Databricks 目录：** 用于镜像来自 Azure Databricks
        的数据以增强集成和分析。
    ...
    j. **镜像的 SQL Server（预览版）：** 用于镜像 SQL Server。

    k. **镜像的 Azure Database for PostgreSQL（预览版）：** 用于镜像您的现有
        Azure Database for PostgreSQL

    l. **镜像的 Azure SQL 托管数据库：** 用于镜像 Azure SQL
        托管数据库以实现高可用性和灾难恢复。

    m. **镜像的数据库（预览版）：** 用于复制数据库以实现高可用性和灾难恢复。
...
2. 系统会将您定向到数据库主页。下面是其各部分的详细概述，旨在帮助您有效地使用
    ...
    **什么是 Fabric 数据库？**
    ...
    Microsoft Fabric 中的 SQL 数据库是一个基于 Azure SQL
    数据库的开发人员友好型交易数据库，它允许您在 Fabric
    中轻松创建运营数据库。Fabric 中的 SQL 数据库使用与 Azure SQL
    数据库相同的 SQL 数据库引擎。

    **项目类型：**

    a. **SQL 数据库（预览版）：** Fabric 中的 SQL
        数据库是数据库工作负荷的一部分，可以从
        Fabric 中的其他项目访问数据。您的 SQL 数据库数据也在 OneLake
        中以可查询格式保持最新，因此您可以使用 Fabric
        中的所有不同服务，例如使用 Spark
        运行分析、执行笔记本、数据工程、通过 Power BI 报表实现可视化等。

    b. **Cosmos DB（预览版）：** Microsoft Fabric 中的 Cosmos DB 是一个 AI
        优化的 NoSQL 数据库，具有简化的管理体验。作为开发人员，您可以使用
        Fabric 中的 Cosmos DB 更轻松地生成 AI
        应用程序，而无需执行典型的数据库管理任务。
    ...
    **入门：**
    ...
    b. **数据库概念：** 解释有关交易数据库的常见术语和概念，以便您可以熟悉如何使用
        SQL
        数据库

    c. **数据库模板：** 浏览公用数据库设计的预先创建的模板库
...
- 阅读[有关 Fabric
  入门的免费电子书](https://aka.ms/fabric-get-started-ebook)

- 加入 [Fabric
  社区](https://aka.ms/fabric-community)发布问题、共享反馈并向他人学习
