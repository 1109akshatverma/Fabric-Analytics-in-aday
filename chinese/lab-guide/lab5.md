...
11. 在显示的 **设置** 面板中，您将看到我们有三个选项：
    ...
    **关于** -
    我们可以在此处更改数据流的名称并添加描述。此外，我们还可以看到谁是数据流的负责人以及上次修改数据流的时间
...
14. 现在，这允许我们通过选择 **重复** 属性的选项来指定刷新的节奏。对于本应用场景，我们可以选择每天
    (1)
...
3. "新建管道"对话框随即打开。将管道命名为  **pl_Refresh_People_SharePoint**，然后选择 **创建**。
    ...
    系统会将您导航到**管道页面**。如果您使用过 Azure
    数据工厂，您会觉得此屏幕很熟悉。让我们快速了解一下布局。
...
7. 从顶部菜单中选择 **视图**。在这里可以找到以 JSON 格式查看代码的选项。还可以找到使活动自动保持一致的选项。
    ...
    > 背景知识，请自行选择"查看 JSON 代码"。
    > 在这里，您会注意到使用设计视图进行的所有编排也可以用 JSON 编写。
...
2. 我们将配置该活动以连接到 df_People_SharePoint 数据流。从底部窗格中，选择"设置"。
...
2. 从顶部菜单中，单击 **+ 新建项目 (1)**，然后从弹出窗口中，单击 **数据管道 (2)**。
...
17. 我们需要将第二个参数添加到 or
    函数中。在结尾的两个括号之间 **添加逗号**。这次我们将尝试输入函数名称。首先输入 **equ**，您将看到可用函数的下拉列表（称为 IntelliSense）。选择
    **equals** 函数。
