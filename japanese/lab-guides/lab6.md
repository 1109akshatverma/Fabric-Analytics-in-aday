...
![A diagram of data flow](../media/lab-6/main6.png)
...
1. ラボ 2 のタスク 8 で作成した Fabric ワークスペース
    **FAIAD_<inject key="Deployment ID" enableCopy="false"/>** に戻りましょう。
...
3. レイクハウス、セマンティック モデル、SQL エンドポイントという 3
    ...
    分析エンドポイントを使用してビジュアル
    クエリを作成しました。左のナビゲーションで **FAIAD_<inject key="Deployment ID" enableCopy="false"/>**
    を選択し、**lh_FAIAD SQL 分析エンドポイント**
    オプションを選択してこのオプションの探索を続けます。エクスプローラーの
    **SQL ビュ**ーが表示されます。
    ...
    クエリです。オプション 2 は T-SQL
    コードの記述です。これは、開発者向けのオプションです。これについて詳しく説明します。
...
4. **上部メニュー**で**新規 SQL クエリ**を選択するか、画面中央で**新規
    SQL クエリ**をクリックします。SQL クエリ ビューが表示されます。
...
9. 保存先のワークスペースがご自身の Fabric ワークスペース
    **FAIAD_<inject key="Deployment ID" enableCopy="false"/>** であることを確認します
...
1. **レイクハウス**を選択すると、SQL 分析エンドポイントからレイクハウス
    ビューに移動します。

    ![](../media/lab-6/image13.png)

2. メニューから、**新しいセマンティック モデル**を選択します。

    ![](../media/lab-6/image14.png)

3. [新しいセマンティック モデル] ダイアログが開きます。Direct Lake
    セマンティック モデル名として **sm_FAIAD** を入力します。

4. 既定でテーブルのサブセットを作成するオプションがあります。前のラボでビューを作成したことを思い出してください。これらのビューをモデルに含めます。**dbo**
    スキーマを展開すると、レイクハウスのすべてのテーブルとビューを確認できます。

    ![](../media/lab-6/image15.png)

5. 次のテーブル/ビューを**選択**します。
...
現在の場所が新しく作成されたセマンティック
モデル内でない場合は、正しい場所に移動しましょう
