...
![A diagram of data flow](../media/lab-5/main5.png)
...
- **顧客データ:** Dataverse で、常に最新の状態です。以前は、このデータを
  1 日に 4 回更新していました (真夜中 (午前 12 時)、午前 6 時、正午
  (午後 12 時)、午後 6 時)。現在では、IT チームによって Dataverse
...
4. **df_Supplier_Snowflake** 行をポイントします。**省略記号 (...)**
    を選択します。
...
6. 前のラボで**保存と実行**オプションを選択したときに実行された 1
    回の更新が表示されていることがわかります。更新の**種類**が**オンデマンド**と表示されていることから、これが手動で実行された更新であることがわかります。
...
8. 右上隅の **X**
    をクリックして、他の場所に移動しましょう。**ワークスペース**に戻ります。

9. **df_Supplier_Snowflake** 行をポイントします。**省略記号 (...)**
    を選択します。
...
11. 表示された**設定**パネルに 3 つのオプションがあることがわかります。
    ...
    > **情報 --**
    > データフローの名前を変更し、説明を追加できます。また、データフローの所有者と最終更新日時も確認できます
    > **承認 --**
    > データフローで他のユーザーが参照できる**昇格済み**または**認定済み**タグを付けるかどうかを指定できます
    > **スケジュール -** データフローのスケジュールを設定できます。
...
14. ここで**繰り返し**プロパティのオプションを選択することで、更新の頻度を指定できます。このシナリオでは、**毎日
    (1)** を選択できます

15. **時間**プロパティには、深夜にするために **12:00 AM (2)**
    を指定できます

    > **注:** [時間の追加]
    > リンクをクリックすると、複数の更新時刻を追加できます。

16. また、**開始日時 (3)** と**終了日時 (4)**
    も指定できます。このシナリオでは、開始日および終了日として今の日付を選択します。

17. どの**タイム ゾーン (5)**
    で日時を表示するかを指定できます。最後に**保存**を選択します
...
3. [新しいパイプライン] ダイアログが開きます。パイプラインに
    ...
    **パイプライン ページ**に移動します。Azure Data Factory
...
7. 上部メニューから、**表示**を選択します。ここには、コードを JSON
    形式で表示するオプションがあります。アクティビティを自動的に位置合わせするオプションもあります。

    **注:** ラボの最後に JSON の予備知識をお持ちの場合は、自由に [JSON
    コードを表示する]
    を選択してください。ここでわかるように、デザイン
...
1. 上部メニューから、**アクティビティ ->
    データフロー**を選択します。データフローアクティビティが中央デザイン
    ペインに追加されます。下部ペインにデータフローアクティビティの構成オプションが表示されることに注目してください。
...
4. **データフロー ドロップダウン**から、**df_People_SharePoint**
    ...
    このシナリオでは、従業員データがスケジュールどおりに更新されません。
    遅延が発生することがあります。これに対応できるかどうか見てみましょう。
...
9. **タイムアウト**を設定するオプションがあります。**既定値**はデータフローを更新するのに十分な時間なので、そのままにしておきます。
    ...
    **注:** スケジュールに基づいたデータがないため、10 分ごとに 3
...
15 分かかります。5 分と 15 分で再試行をスケジュールする必要があります。
新しいデータ
...
11. 同様の手順に従って、さらに 3 つの変数を追加します。
    ...
    a. **varIsSuccess:** 種類は String、既定値は **No** に設定します。
    ...
    c. **varWaitTime:** 種類は Integer、既定値は **60**
...
6. **式テキスト ボックス (2)** を選択します。このテキスト
    ボックスに、true または false
    と評価される式を入力する必要があります。Until
    アクティビティは、この式が false
...
7. テキスト ボックスの下に表示される**動的なコンテンツの追加 (3)**
    ...
    の値が 3**
    であるか、**varIsSuccess の値が Yes** である。(varCounter と
...
**注:** 関数は **@or(equals())** のようになるはずです。equals 関数も 2
...
17. or 関数に 2 番目のパラメーターを追加する必要があります。最後の 2
    ...
    と入力し始めると、
    使用できる関数のドロップダウンが表示されます (IntelliSense
    と呼ばれます)。
    **equals** 関数を選択します。
...
4. **データフロー
    アクティビティを選択した**状態で、下部ペインで**全般**を選択します。
...
10. パイプライン式ビルダー
    ダイアログが開きます。**式、関数、システム変数の任意の組み合わせを使用して以下に動的コンテンツを追加しますのテキスト領域
    (1)** を選択します。

11. 下部メニューの**省略記号 (...) (2)** をクリックし、**変数 (3) ->
    varSuccess (4)**
...
12. **OK** を選択します。**反復処理デザイン ペイン**に戻ります。
    ...
    パイプラインでは、変数を自己参照することはできません。
    つまり、カウンター変数 varCounter の値に 1
    を加えて増分することはできません (varCounter = varCounter +
    1)。そこで、varTempCounter 変数を利用します。
...
5. データフロー アクティビティの**赤い x
    マーク**をクリックして、新しい変数の設定アクティビティに接続します。つまり、データフローの更新が失敗したら、
...
10. パイプライン式ビルダー
    ...
    **注:**
    ...
    varTempCounter の値を、変数 varCounter の値に 1
    を加えたものに設定します (varTempCounter = varCounter + 1)。
...
10. パイプライン式ビルダー
    ...
    と入力します。この式を入力しても、メニューを使って関数を選択しても、
    またはコピーして貼り付けても構いません。
...
2. **Wait
    アクティビティ**を選択した状態で、下部ペインで**全般**を選択します。
    アクティビティの名前と説明を指定しましょう。
...
activity](../media/lab-5/image50.png)

10. デザイン キャンバスの左上で **pl_Refresh_People_Sharepoint_Option2**
    またはメイン キャンバスを選択して、Until 反復処理から移動します。
...
1. データ パイプラインをテストするには、**ホーム ->
    ...
    **注:** データ
    パイプラインの更新が完了するまでに数分かかる場合があります。
    これはトレーニング環境であるため、SharePoint
    のファイルは常に利用可能です。
    したがって、データ パイプラインが失敗することはありません。
...
にフィードバックを提供するためにのみデモ/ラボを使用できます。それ以外の目的には使用できません。このデモ/ラボまたはその一部を、変更、コピー、配布、送信、表示、実行、
再現、発行、ライセンス、著作物の作成、転送、または販売することはできません。
...
テクノロジ/製品の機能を提供します。このデモ/ラボで表されるテクノロジ/概念は、
フル機能を表していない可能性があり、最終バージョンと動作が異なることがあります。また、そのような機能や概念の最終版がリリースされない場合があります。
物理環境でこのような機能を使用するエクスペリエンスが異なる場合もあります。
...
は、デモ/ラボから派生する結果、出力の正確さ、任意の目的に対するデモ/ラボに含まれる情報の適合性に関して、
いかなる保証または表明もしません。

**免責事項**

このデモ/ラボには、Microsoft Power BI
の新機能と機能強化の一部のみが含まれています。一部の機能は、製品の将来のリリースで変更される可能性があります。
