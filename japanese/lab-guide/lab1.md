...
データから貴重な分析情報を引き出すには、さまざまなシステムからデータを取得し、
...
モデルを構築しています。その目的は、自分のレポート作成のニーズを満たし、
エンド ユーザーにセルフ サービス機能を提供することです。モデルの更新には
Power Query を使います。

**直面している課題:**

- 各種データ ソースごとに異なる更新時間に対応するには、1 日に少なくとも
  3 回はデータセットを更新する必要があります。
...
1. お使いのラボ環境のデスクトップにある **Reports** フォルダー内の
    **FAIAD.pbix** を開
    きます。**Power BI Desktop** でファイルが開かれます。
...
7. 次のダイアログで、**パスワード**の**資格情報を環境の詳細**タブからコピーして
    入力します。
...
9. **すべてのアプリにサインインしたままにする**ダイアログが開きます。**OK**
    を選択します。

    ![](../media/lab-1/image10.png)

10. **準備が完了しました。**ダイアログが開きます。**完了**を選択します。
...
7. 製品の詳細を提供するページに移動します。今後の注文もいくつかあることがわか
    ります。
...
11. **ADLSData** フォルダーには複数のディメンション
    ...
    - **Geo ディメンション**は、Cities、Countries、States
      の各クエリからのデータをマージすることで作成されます。

    - **Product ディメンション**は、Product Groups と Product Item Group
      のクエリからのデータをマージすることで作成されます。

    - **Reseller ディメンション**は、BuyingGroup クエリを使ってフィルター処
    理されます。
...
17. **SharepointData** フォルダーには People
    ...
    Gen2 を使って同様の Power Query
    を作成し、レイクハウスを使ってモデリングを行います。
