...
![A diagram of data flow](../media/lab-4/main4.png)
...
3. **データの取得 (3)** セクションで、**Dataflow Gen2 (4)**
    ...
    既定の名前のままにして [作成]
    を選択します。データフローのページが表示されます。データフロー (Gen2)
    のインターフェイスは、
...
7. **右クリック**して**コピー**を選択します。
    ...
    ![P54#yIS1](../media/lab-4/image7.png)
...
9. **データフローのペイン**で、**Ctrl+V** を押します
    ...
    **VM ネイティブ クリップボード**を**有効**にします。ダイアログで [OK]
    ...
    ![P58#yIS1](../media/lab-4/image9.png)
...
4. **接続**を選択します。
    ...
    **注:**
    ...
    ![P67#yIS1](../media/lab-4/image11.png)

## タスク 3: People クエリのデータのコピー先を構成する

接続が確立され、プレビュー パネルにデータが表示されます。クエリの
[適用されたステップ] を自由に見て回りましょう。次は、People
...
1. **Power Query
    のウィンドウ**が表示されます。**右下隅**を見ると、[データ同期先]
    がレイクハウス **(1)** に設定されています。

2. 左上隅の**保存と実行 (2)**
    を選択します。更新が開始されたという通知が表示されたら、データフローを閉じることができます
    **(3)**
...
3. **Dataflow 1**
    ...
    1 の横にある**省略記号 (...)**
    をクリックします。**設定**を選択します
    (データフローの実行中は設定にアクセスできません)。

    ![](../media/lab-4/image17.png)

4. データフロー設定ウィンドウが開きます。**名前**を
    **df_People_SharePoint** に変更
    します (1)。
...
3. 推奨項目から **データフロー (Gen2) (3)** を選択します。
    ...
    既定の名前のままにして**作成**を選択します。"この名前のデータフローは既に存在します"
    というメッセージが表示された場合は、名前を **Dataflow 2**
    に変更します。**データフロー
    ページ**が表示されます。ここまでデータフローについて説明してきました。次は、Power
...
9. **データフローのペイン**で**中央のペイン**を選択し、**Ctrl+V**
    ...
    **注:** ラボ環境で作業している場合は、画面の右側にある**省略記号
    ...
    ![P121#yIS1](../media/lab-4/image22.png)
...
3. [データ変換先に接続]
    ダイアログが開きます。**接続ドロップダウ**ンから **Lakehouse
    odl_user_<ユーザー名> (なし)** を選択します。
...
1. 画面上部で、**Dataflow 2 (名前は異なる場合があります)
...
7. サインイン ダイアログ
    ...
    ![P182#yIS1](../media/lab-4/image37.png)
...
8. [接続設定] ダイアログで、**次へ**を選択します。
    ...
    (Gen2) を使用して SharePoint に接続することもできます。
...
![P214#yIS1](../media/lab-4/image45.png)
