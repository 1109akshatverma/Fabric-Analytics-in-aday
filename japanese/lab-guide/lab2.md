...
1. **ブラウザー**を開いて、[Microsoft Power BI
    ...
    ***注:** ラボ環境を使用しておらず、Power BI
    ...
    モードまたはシークレット モードで使用することをお勧めします。*
...
9. Microsoft Fabric
    の無料試用版にアップグレードするダイアログが開きます。
    **アクティブ化**を選択します。

    ***注:**
    リージョンは既定値のままにします。講師は、ラボを振り返る際に、これについてさらに詳しく説明します*
...
11. **Microsoft Fabric のホーム** **ページ**が開きます。[Fabric
    ビューへようこそ]
    ダイアログが開く場合があります。必要に応じて、**ツアーの開始**オプションを選択するか、キ**ャンセル**を選択できます。
...
4. **名前**フィールドに **FAIAD_<inject key="Deployment ID" enableCopy="false"/>** ([環境]
    タブで見つけることができます)
    を入力します
...
8. **適用**を選択して新しいワークスペースを作成します。
    ...
    新しいワークスペースが作成されて、そのワークスペースに移動できます。さまざまなデータ
    ソースからレイクハウスにデータを取り込み、そのレイクハウスからのデータを使用してモデルを構築し、レポートを作成します。最初のステップでは、レイクハウスを作成します。次にこれを行います。
...
3. 新しいレイクハウスのダイアログが表示されます。[名前]
    ...
    ***注:** ここで、lh はレイクハウスを表します。lh
    を接頭辞として付けることで、識別しやすくなり、検索が容易になります。*
    
    ***注:** プレビュー機能である**レイクハウス
    スキーマ**は、非常に興味深い機能であるため、知っておいてください。これは**プレビュ**ー状態であるため、ラボ環境に悪影響がないように無視します。機能が一般提供になったら、このラボに組み込みます。*

4. **作成**を選択します

    ![](../media/lab-2/image17.png)

    しばらくするとレイクハウスが作成され、レイクハウス
    エクスプローラーのインターフェイスが表示されます。左上にあるヘッダーの
    Fabric
    名の横に、レイクハウスのアイコンが表示されます。左側のナビゲーションのワークスペース
    アイコンに、項目が含まれていることが反映されます

    レイクハウス エクスプローラーには Tables と Files
    のセクションがあります。レイクハウスで [Files] セクションの下に Azure
...
1. 画面左側のワークロード
    ...
    Engineering、Data
    Science、Data Warehouse が含まれます。詳しく見ていきましょう。
...
3. Data Factory のホーム ページが表示されます。以下は、Data Factory
    ...
    **項目の種類:**
    ...
    a) **データフロー Gen2:**
    データフローは、データ変換のためのレシピのようなものです。データに適用できる
    300
    種類以上の変換が提供されています。つまり、ニーズに合わせてデータをさまざまな方法でクリーニング、結合、変更できます。
    ...
    d) **Apache Airflow ジョブ:** Apache Airflow
    ...
    f) **ミラー化されたデータベース:**
    バックアップ、テスト、または読み取り専用アクセス用に、ミラーリングされたバージョンのデータベースを作成する機能。

    g) **ミラー化された Oracle (プレビュー):** Fabric
    でのミラーリングにより、Oracle
    データベースが統合プラットフォームにレプリケートされ、他のデータ
    ソースとともに準リアルタイムで低遅延の分析が可能になります。

    h) **ミラー化された Google BigQuery (プレビュー)**: Fabric
    でのミラーリングにより、Google BigQuery データを OneLake
    に継続的にレプリケートできるため、複雑な ETL
    が不要になり、分析、AI、データ共有にまたがったシームレスな使用が可能になります。

    i) **変数ライブラリ (プレビュー):**
    変数とその既定値の一覧が含まれています。また、代替値を保持している他の値セットを含めることもできます
    ...
    **作業の開始**
    ...
    このセクションには、最初のデータ
    パイプラインを作成する方法が示されています。パイプラインを使用すると、データ
    プロセスを効率的に自動化および管理できます。

    d) **Data Factory を監視する方法を学びます:** 監視は、データ
    プロセスを円滑に実行
    するために不可欠です。このセクションには、Data Factory
    の活動を監視する方法が示
    されています。
    ...
    g) **最初のユーザー データ関数を作成する:** このセクションは、ユーザー
    データの管理と変換に便利なユーザー
    データ関数を作成するのに役立ちます。

    ![](../media/lab-2/image21.png)

4. 画面の左上隅にある**ワークロードに戻る**をクリックします。このアクションによって主要なワークロードのページが表示され、そこで他のツールやセクションを探索 できます。
...
2. Industry Solutions のホーム ページが表示されます。以下は、Industry
    ...
    **作業の開始:**
    ...
    ボタンをクリックして持続可能性ソリューションについて読み、プロジェクトでどのように使用できるのかを理解
    します。
...
2. Real-Time Intelligence のホーム
    ...
    **項目の種類:**
    ...
    f. **イベント スキーマ セット (プレビュー)**:
    リアルタイム分析ワークフローのデータ構造 (スキーマ)
    の整理および標準化に役立ち、ストリーミング
    データを一貫して処理および分析することが容易になります。

    g. **デジタル ツイン ビルダー (プレビュー):** デジタル ツイン
    ビルダーは、オントロジーを通じて、コーディングの経験が浅いまたはまったくないユーザーが、アセットやプロセスなどのビジネス概念を構築してモデル化するのを支援します。

    h. **マップ (プレビュー)**: 地理空間の分析情報を Real-Time Intelligence
    に取り入れることで、だれでもイベントの発生場所を視覚化し、空間データを他の
    Fabric
    機能と統合して、よりスマートで場所を認識した意思決定ができるようになります。

    **作業の開始:**
    Real-Time Intelligence の使用を開始するには、次の手順に従います。

    a. **リアルタイムでのエンドツーエンド エクスペリエンス**: [作業の開始]
    ボタンをクリックして、サンプル データ セットを使用してリアルタイム
    データ分析を探索します。

    b. **Real-Time Intelligence サンプルを探索する**: [開く]
    ボタンをクリックし、サンプルを使用してリアルタイム
    データ分析を探索します。

    c. **イベントハウス サンプルの探索:** [選択]
    ボタンをクリックし、サンプルを使用して Real-Time Intelligence
    について学びます。

    d. **Real-Time Intelligence の概要:** [開く]
    ボタンをクリックして、Real-Time Intelligence
    の概要を表示し、このツールの効果的な使用を開始します。

    e. **サンプル データを使用して KQL を学習する:** [開く]
    ...
    g. **サンプルのアクティベーターを探索する:** [開く]
    ボタンをクリックし、サンプルのアクティベーターを使用して Real-Time
    Intelligence の機能について理解します。

    h. **アクティベーターの使用を開始する:** [開く]
    ボタンをクリックして、アクティベーターの概念について概要を確認し、ツールの効果的な使用を開始します。
...
2. **Data Engineering** のホーム ページが表示されます。以下は、**Data
    ...
    **項目の種類:**
    ...
    e. **ユーザー データ関数 (プレビュー):** Fabric
    でアプリケーションをホストおよび実行できるプラットフォームです。

    f. **GraphQL 用 API:** 複数のデータ ソースに対してクエリを実行する API
    です。
...
2. **Data Science** のホーム ページが表示されます。以下は、**Data
    ...
    **項目の種類:**
    ...
    e. **データ エージェント (プレビュー):**
    レイクハウス、ウェアハウス、Power BI セマンティック モデル、KQL
    データベースの格納データに関する質問に答える、会話型 AI
    エクスペリエンスを作成するために使用されます。

    f. **Python Notebook:** ローカル マシンから Python
    ノートブックをインポートするために使用されます。

    **作業の開始:**
    Data Science の使用を開始するには、次の手順に従います。

    a. **サンプルの探索:** [選択] ボタンをクリックし、サンプルを使用して
    Data Science について学びます。

    b. **機械学習モデルの概要:** [開く]
    ボタンをクリックして、機械学習モデルの使用を開始する方法を学びます。

    c. **機械学習実験の概要:** [開く]
    ボタンをクリックして、機械学習の実験を実施する方法を学びます。
...
3. 画面の左上隅にある**ワークロードに戻る**をクリックします。このアクションによって主要なワークロードのページが表示され、そこで他のツールやセクションを探索
    できます。
...
2. Data Warehouse のホーム ページが表示されます。以下は、Data Warehouse
    ...
    **項目の種類:**
    ...
    d. **ミラー化された Azure SQL Database:** Azure SQL Database
    をミラーリングするために使用されます。

    e. **ミラー化された Azure Databricks カタログ:**
    統合や分析の強化のために Azure Databricks
    からデータをミラーリングするのに使用されます。

    f. **ミラー化された Snowflake:** Snowflake
    データベースをミラーリングするために使用
    されます。

    g. **ミラー化されたOracle (プレビュー):** Oracle
    をミラーリングするために使用されます。

    h. **ミラー化された Google BigQuery (プレビュー):** Google BigQuery
    をミラーリングするために使用されます。

    i. **ミラー化された Azure Cosmos DB:** Azure Cosmos DB
    をミラーリングするために使用されます。

    j. **ミラー化された SQL Server (プレビュー):** SQL Server
    をミラーリングするために使用されます。

    k. **ミラー化された Azure Database for PostgreSQL (プレビュー):**
    既存の Azure Database for PostgreSQL
    をミラーリングするために使用されます。

    l. **ミラー化された SQL Managed Database:** 高可用性とディザスター
    リカバリーのために Azure SQL Managed Database
    をミラーリングするのに使用されます。

    m. **ミラー化されたデータベース (プレビュー):** 高可用性とディザスター
    リカバリーのためにデータベースを複製するのに使用されます。

    **作業の開始:**
    Data Warehouse の使用を開始するには、次の手順に従います。

    a. **サンプル ウェアハウスの探索:** あらかじめ読み込まれているサンプル
    データを使用して、新しいウェアハウスを開始します。
...
2. Databases のホーム ページが表示されます。以下は、Databases
    ...
    **項目の種類:**
    ...
    b. **Cosmos DB (プレビュー):** Microsoft Fabric の Cosmos DB
    は、シンプルな管理エクスペリエンスを提供する、AI によって最適化された
    NoSQL データベースです。開発者は、Fabric の Cosmos DB
    を使用することで、一般的なデータベース管理タスクを実行することなく、ストレスを少なくして
    AI アプリケーションを構築できます。
