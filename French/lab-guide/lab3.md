...
6. Cliquez sur **Nouvelle connexion (1)**.
...
14. Vous êtes alors redirigé(e) vers la boîte de dialogue suivante dans
    laquelle nous pouvons modifier
    les noms. Cliquez sur l'**icône Modifier (1)** sous Actions pour
    **Application.Cities**.
...
19. Notez que tous les raccourcis sont créés sous forme de tables.
    Sélectionnez la table **BuyingGroups**
    et notez que nous pouvons voir une version préliminaire des données
    dans le volet de données.
...
13. Un volet s'ouvre alors. Assurez-vous que seules les colonnes
...
21. Un volet s'ouvre alors. Assurez-vous que seules les colonnes
    suivantes sont sélectionnées :
...
29. Cliquez sur **Enregistrer** en bas à droit de l'Éditeur Power Query.
    ...
    **Remarque** **:** toutes les étapes que nous avons effectuées à l'aide
    de l'éditeur Power Query peuvent également être réalisées à l'aide de
...
4. **Sélectionnez la requête** Customers. Une fois sélectionnée, la
    ...
    indique que nous ajoutons une étape après Table. Si vous ne voyez
    pas le signe « **+** » après Table, vous avez peut-être sélectionné
    une autre étape. Sélectionnez Table et vous serez prêt.)
...
l'aide d'une requête visuelle, car
la création d'une requête vide n'est pas possible dans la requête
visuelle. Essayons cette méthode.
...
24. À l'aide du raccourci clavier **Ctrl + V**, collez le code que vous
    ...
    **Remarque** **:** si vous travaillez dans l'environnement de labo,
    ...
    en haut de l'écran à droite. Utilisez le curseur pour **activer** **le
    Presse-papiers natif de VM**. Cliquez sur OK dans la boîte de dialogue.
    Après avoir collé les requêtes, vous pouvez désactiver cette option.
...
TOUTE PARTIE
DE CEUX-CI) SUR TOUT AUTRE SERVEUR OU AUTRE EMPLACEMENT EN VUE D'UNE
AUTRE REPRODUCTION OU REDISTRIBUTION EST EXPRESSÉMENT INTERDITE.
