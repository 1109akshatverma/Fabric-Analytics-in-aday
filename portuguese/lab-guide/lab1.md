...
- **Dados de Vendas:** são obtidos do sistema ERP e armazenados em um
  banco de dados ADLS Gen2. Eles são atualizados ao meio-dia/12h, todos
  os dias.
...
- Você precisa atualizar seu conjunto de dados pelo menos três vezes por
  dia para acomodar os diferentes horários de atualização para as
...
- Os erros detectados em qualquer uma das fontes das quais você está
  ...
  prazo, resultando na interrupção da atualização do conjunto de dados.
...
10. **Tudo pronto!** A caixa de diálogo é aberta. Selecione
    **Concluído**.
...
4. No visual Gráfico de dispersão, na parte inferior central da tela
    (Sales Orders por Sales), selecione a exceção no canto superior
    direito (4º quadrante). Observe que a margem percentual é de 52%,
...
11. Observe que a pasta **ADLSData** tem múltiplas dimensões: Geo,
    ...
    - **Dimensão Product** é criada mesclando dados da consulta Product
      Groups e Product Item Group.

    - **Dimensão Reseller** é filtrada usando a consulta BuyingGroup.

    - **Fato Sales** é criado mesclando InvoiceLineItems com a consulta
      Invoice.
...
14. A pasta SnowflakeData tem a dimensão Supplier e o fato PO
    ...
    - A **dimensão Supplier** é criada mesclando a consulta Suppliers com a
    consulta SupplierCategories.
