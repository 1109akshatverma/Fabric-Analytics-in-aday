...
1. Vamos voltar ao **workspace do Fabric** que você criou no
    laboratório 2, denominado **FAIAD_<inject key="Deployment ID" enableCopy="false"/>.**
...
5. Você será direcionado para **Criar seu primeiro relatório**. Haverá
    ...
    modelo semântico nos laboratórios anteriores, portanto, vamos
...
10. A caixa de diálogo Salvar seu relatório é aberta. Nomeie o relatório
    ...
    **Observação:** estamos prefixando o nome do relatório com rpt, que é a
    abreviação de relatório.
...
6. Se você ainda não tiver aberto, abra o arquivo **FAIAD.pbix** que
    está na pasta **Reports** na  **área de trabalho** do seu ambiente
    de laboratório.
...
7. No painel **Propriedades**, à direita, expanda a seção **Avançado**.
...
10. No painel **Dados** da direita, expanda a tabela **Date**. Observe
    que Year não é campo de soma.

11. Com o **Visual de gráfico de linhas selecionado**, **remova Soma de
    Year** do eixo Y.
...
1. Vamos adicionar Month a este gráfico. Na tabela Date, arraste o
    ...
    que o visual é classificado por Sales. Vamos
    classificá-lo por **MonthNameShort**.
...
2. No canto inferior direito, clique no **logotipo do Fabric** e
    alterne para a **exibição do Fabric**.

3. Selecione **FAIAD_<inject key="Deployment ID" enableCopy="false"/>** na barra de menus à
...
9. Se você não criou essa conexão anteriormente no curso, clique em
    Criar nova conexão e conclua as seguintes etapas:

10. Em Configurações de conexão -> URL, insira este link:
...
17. Selecione a **marca de seleção** ao lado do nome para salvar a
    alteração.
...
21. **Copie** o código abaixo e **cole-o** no painel da consulta
    SQL**.**
...
22. No menu de consulta visual, selecione **Executar** para executar o
    código.
...
24. No menu superior, selecione **Atualizar**. Observe agora no gráfico
    ...
    - **As operações de atualização podem demorar, pois é sempre necessário
    fazer uma atualização completa para capturar tudo o que foi atualizado
...
1. Selecione o workspace **FAIAD_<inject key="Deployment ID" enableCopy="false"/>** no painel
