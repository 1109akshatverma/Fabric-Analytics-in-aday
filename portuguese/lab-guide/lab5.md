...
- **Dados do Cliente:** no Dataverse, são sempre atualizados.
  ...
  meia-noite/24h, 6h, meio-dia/12h e 18h. Agora, a equipe de TI criou um
  link para o Dataverse ingerir esses dados em um Lakehouse
  administrativo. Eles também transformaram esses dados. Não precisamos
  configurar a atualização, pois estamos vinculando ao Lakehouse
  fornecido pela equipe de TI.

- **Dados de Venda**: no ADLS são atualizados ao meio-dia/12h todos os
  dias. Não precisamos configurar a atualização para isso, pois criamos
  um atalho. Os dados estarão disponíveis assim que forem atualizados no
  ADLS.
...
4. Passe o mouse sobre a linha **df_Supplier_Snowflake**. Selecione as
    **reticências (...)**.

5. Observe que há opção para Excluir, Abrir e Atualizar o Fluxo de
    Dados. Vejamos o Histórico de atualização. Selecione **Execuções
    recentes**.
...
6. Você notará que há uma atualização singular executada quando
    selecionamos a opção **Salvar
    e Executar** no laboratório anterior. O **Tipo** de atualização que
    podemos ver está listado como **Sob demanda**, o que nos permite
    saber que esta foi uma atualização executada manualmente.
...
8. Vamos sair, clicando no **X** no canto superior direito. Você
    voltará para o **espaço de trabalho**.

9. Passe o mouse sobre a linha **df_Supplier_Snowflake**. Selecione as
    **reticências (...)**.

10. Vamos ver como podemos agendar uma atualização para que ela aconteça
    automaticamente. Escolha a opção **Configurações**.
...
14. Isso agora nos permite especificar a cadência da atualização
    selecionando uma opção para a propriedade **Repetir**. Neste
    cenário, podemos escolher **Diariamente (1)**

15. Para a propriedade **Hora do dia**, podemos especificar **00:00
    (2)**, pois nós queremos meia-noite

    **Observação:** ao clicar no link Adicionar outro horário, você pode
    adicionar vários horários de atualização.

16. Também podemos especificar uma **Data e hora de início (3)**, bem
    como uma **Data e hora de término (4)**. Para este cenário, basta
    escolher o dia atual para a data de início e a data de término.

17. Você pode especificar qual **Fuso horário (5)** você gostaria que os
    horários representassem. Por fim, selecione **Salvar**

    ![](../media/lab-5/image15.png)

18. Você verá a atualização agendada e poderá editá-la ou excluí-la se
    não for mais necessária ou adicionar outras atualizações agendadas.
...
3. Uma nova caixa de diálogo de pipeline é aberta. Nomeie o pipeline
    como **pl_Refresh_People_SharePoint** e selecione **Criar**.

    ![](../media/lab-5/image18.png)

    Você direcionado para a **página Pipeline**. Se você trabalhou com o
    Azure Data Factory, esta tela será familiar. Vamos obter uma visão geral
    rápida do layout.

    Você está na tela **Início**. Se você olhar o menu superior, encontrará
    opções para adicionar as atividades comumente usadas: validar e executar
    um pipeline e visualizar o histórico de execuções. Além disso, no painel
    central, você encontrará opções rápidas para começar a criar o pipeline.
...
7. No menu superior, selecione **Exibir**. Aqui você encontrará opções
    ...
    opções para alinhar automaticamente as atividades.
...
2. Vamos configurar a atividade para conectar-se ao fluxo de dados
...
9. Há uma opção para definir **Tempo limite**. Vamos deixar o **valor
    ...
    **Observação:** como os dados não estão disponíveis em uma agenda, vamos
...
13. No menu, selecione **Página Inicial -> Salvar** para salvar o
    ...
    - O pipeline oferece a opção de tentar novamente várias vezes antes de a
    atualização falhar.

    - O pipeline fornece a capacidade de executar outras tarefas, além de
...
8. A caixa de diálogo Construtor de expressões de pipeline é aberta.
    Insira
...
10. Na parte superior esquerda da tela de design, selecione
    **pl_Refresh_People_Sharepoint_Option2** ou **Main Canvas** para
    sair do iterador Until.
...
2. Podemos definir o pipeline de dados para ser executado de acordo com
    um agendamento. No menu superior, selecione **Página Inicial >
    Agendamento**. A caixa de diálogo Agendamento é aberta.

3. Selecione o botão **Adicionar agendamento** abaixo de **Execução
    agendada**.
